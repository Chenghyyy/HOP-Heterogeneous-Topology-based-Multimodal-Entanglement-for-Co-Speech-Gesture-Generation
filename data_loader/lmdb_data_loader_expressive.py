    ...
    def __getitem__(self, idx):
        ...
        def extend_word_seq(lang, words, end_time=None):
            ...
            if self.remove_word_timing:
                ...
                text = ' '.join(text)
                text_token = self.tokenizer(text, return_tensors="pt", padding=False, truncation=True, max_length=2048,
                                            add_special_tokens=False).input_ids
                # print(text)
                # print(text_token)
                # print('text_token.shape',text_token.shape)
                for word in words:
                    if n_words == text_token.shape[1]:
                        break
        ...
        return word_seq_tensor, extended_word_seq, text_token_padded, text, pose_seq, vec_seq, audio, log_melspec, spectrogram, aux_info
