...
def convert_dir_vec_to_pose(vec):
    ...
    if len(vec.shape) == 2:
        joint_pos = torch.zeros((43, 3)).to(device)
        for j, pair in enumerate(dir_vec_pairs):
            joint_pos[pair[1]] = joint_pos[pair[0]] + torch.Tensor([pair[2]]).to(device) * vec[j]
    elif len(vec.shape) == 3:
        joint_pos = torch.zeros((vec.shape[0], 43, 3)).to(device)
        for j, pair in enumerate(dir_vec_pairs):
            joint_pos[:, pair[1]] = joint_pos[:, pair[0]] + torch.Tensor([pair[2]]).to(device) * torch.Tensor(vec[:, j]).to(device)
    elif len(vec.shape) == 4:  # (batch, seq, 42, 3)
        joint_pos = torch.zeros((vec.shape[0], vec.shape[1], 43, 3)).to(device)
        for j, pair in enumerate(dir_vec_pairs):
            joint_pos[:, :, pair[1]] = joint_pos[:, :, pair[0]] + torch.Tensor([pair[2]]).to(device) * vec[:, :, j]
    else:
        assert False
